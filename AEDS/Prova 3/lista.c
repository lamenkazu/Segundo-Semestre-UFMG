...
void lista_insereInicio(lista_t * lista, infoLista_t item) {
  ...
  novo->info = (infoLista_t*) malloc(sizeof(infoLista_t));
  
  //Não permite inserir elementos duplicados
  for(itemLista_t * i = lista->primeiro; i != NULL; i = i->prox){
    if(strcmp(i->info->caracteres, item.caracteres) == 0){
      return;
    }
  }

  strcpy(novo->info->caracteres, item.caracteres);
  novo->info->inteiro = item.inteiro;
...
void lista_imprime(lista_t * lista) {
  ...
  for (atual = lista->primeiro; atual != NULL; atual = atual->prox) {
    printf("{%s, %d}\n ", atual->info->caracteres, atual->info->inteiro);
  }
}
...
itemLista_t * lista_busca(lista_t * lista, infoLista_t valor) {
  ...
  for (atual = lista->primeiro; atual != NULL; atual = atual->prox) {
    ...
    if(strcmp(atual->info->caracteres, valor.caracteres) == 0){
      return atual;
    }
...
bool_t lista_retira(lista_t * lista, infoLista_t valor) {
  ...
  while ((atual != NULL) && (strcmp(atual->info->caracteres, valor.caracteres) != 0)) {
    anterior = atual;
    atual = atual->prox;
  }
